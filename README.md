...
- **PathKeeper** : 실시간 위치 추적 플랫폼 (2026.03 ~ )
  ...
  - `Kafka` 기반 이벤트 처리: 위치 이벤트를 비동기로 수신하여 응답성 확보 및 userId 기반 파티셔닝으로 데이터 순서 보장 및 수평 확장성 확보
  ...
  - `모니터링`: PLG + Alloy 모니터링 스택을 통해 부하 테스트 중 peak Load Average(최대 53)를 관찰하고, CPU 자원 한계로 인해 p99 지연이 발생했음을 진단
- **영화 예매 서비스** : (2026.03 ~ 2026.05)
  ...
  - `커넥션 풀 최적화`: PLG + Alloy 모니터링 스택을 활용해 DB 커넥션 풀 고갈 지점을 포착하고, 커넥션 풀 설정 최적화를 통해 Latency spike 해결
